...
do_propegate_message(State, Ref, ClientPID, Message) ->
	...
	lists:map(fun(X) -> X!{request, self(), Ref, 
							{incoming_msg, NickName, State#chat_st.name, Message}
						}
				end,
				lists:filter(fun (ChatPID) -> (ChatPID/=ClientPID) end, maps:keys(State#chat_st.registrations))
	),
	ClientPID!{self(), Ref, ack_msg},
	State#chat_st{
		name=State#chat_st.name,
		registrations=State#chat_st.registrations,
		history=State#chat_st.history++[{NickName, Message}]
	}.
